...
3. Write (or create) `<run_dir>/audit-[n].json` with a `description_optimizer` block: `{ "skipped": true, "skip_reason": "<reason>", "original_description": "<value or null>", "final_description": "<original or null>", "improved": false }`.
...
Check whether `<run_dir>/audit-[n].json` exists. If it exists, read it and append the `description_optimizer` block. If it does not exist, create it with only the `description_optimizer` block (the lint step may have been skipped). If reading or writing `audit-[n].json` fails, emit `⚠ Could not write optimizer results to audit-[n].json: <error>` and end the run — do not re-raise. Build the `description_optimizer` block: `original_description`, `final_description` (best if improved, else original), `improved`, `iterations_run`, `train_set_size: 12`, `validation_set_size: 8`, `baseline_train_pass_rate`, `best_train_pass_rate`, `baseline_val_pass_rate`, `best_val_pass_rate`, `selected_iteration`, `iteration_log` (id, description excerpt ≤80 chars, train_pass_rate, call_count, failures). Write `audit-[n].json`. Emit a summary table. End the run.
