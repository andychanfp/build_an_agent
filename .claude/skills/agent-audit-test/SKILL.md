...
**Invoke**: handed off from agent-audit with `skill_path`, `run_dir`, `schemas_path`, `mode` args.
...
Spawn one Agent subagent per test case in a single message so they run concurrently. Each subagent prompt must: (1) tell the subagent to act as a protocol executor — read the target skill's SKILL.md and follow its step-by-step protocol directly, not trigger it as a registered skill via the Skill tool; (2) include the full test prompt (including any image URL from Step 1); (3) instruct the subagent to return its complete, verbatim output as text in the completion message — no file writes, no summarising. Wait for all subagents to complete. Collect `actual_output` as the full verbatim text returned by each subagent and `timing` (`total_tokens`, `duration_ms`) per case from the completion metadata. If a subagent returns no output, record `actual_output: null` and `timing: null` for that case, emit `⚠ eval-[id] returned no output — continuing.`, and proceed. Write the full `evals-[n].json` array (with verbatim `actual_output` per case) to `run_dir` in one write. Produce `evals_path = <run_dir>/evals-[n].json`.
